...
def backbone_net_agtcnet(args, return_type='xy'):
    """
    AGTCNet returns (features, logits) directly, similar to FBCNet and Conformer
    So we only return netF without a separate classifier
    """
    try:
        F1, D = args.F1, args.D
    except:
    ...
    netF = AGTCNet(
        ...
        F1=F1,
        D=D,
        kernLength=int(args.sample_rate // 2) if hasattr(args, 'sample_rate') else 32,
        dropout=args.dropoutRate if hasattr(args, 'dropoutRate') else 0.5,
        feature_dim=args.feature_deep_dim  # Add feature adapter to match expected dimension
    )
    # AGTCNet returns both features and logits, so we don't need a separate classifier
    # Return only netF (no netC needed)
    return netF
...
def waveformer_encoder(args, nlayer=1):
    """WaveFormer Encoder - replaces standard Transformer"""
    from models.waveformer_adapter import WaveFormerBlock

    # Calculate temporal length from dim_p
    # dim_p = feature_dim * temporal_len
    # For AGTCNet: feature_dim = 224, temporal_len varies by reshape
    # We'll use dim_e as the feature dimension

    blocks = nn.ModuleList([
        WaveFormerBlock(
            dim=args.dim_e,
            temporal_len=args.dim_p // args.dim_e if hasattr(args, 'dim_p') else 22,  # Approximate
            dropout=0.1,
            mlp_ratio=4.0
