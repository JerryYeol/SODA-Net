...
def cr_transform(args, X, y):
    ...
    elif args.data_name == 'Lee2019':
        # 19 sensorimotor channels, order:
        # FC5 FC3 FC1 FC2 FC4 FC6 | C5 C3 C1 Cz C2 C4 C6 | CP5 CP3 CP1 CPz CP2 CP4
        # 左右半球镜像配对 (CP5 无 CP6 对应, Cz/CPz 为中线, 均保持不变)
        left_mat = [0, 1, 2, 6, 7, 8, 14, 15]
        right_mat = [5, 4, 3, 12, 11, 10, 18, 17]
    num_samples, num_channels, num_timesamples = X.shape
    transformedX = np.zeros((num_samples, num_channels, num_timesamples))
    ...
    if args.data_name == 'Zhou2016' or args.data_name == 'MI1-7' or args.data_name == 'BNCI2014001' or 'BNCI2014004' in args.data_name or args.data_name == 'Lee2019':
        labels = 1 - y
    elif args.data_name == 'BNCI2014001-4' or args.data_name == 'BCICIV2a':
        # 4类：左右镜像交换 left_hand(0) 和 right_hand(1)，feet(2)/tongue(3) 不变
