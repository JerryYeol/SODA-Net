...
def data_process(args, dataset):
    ...
    if dataset == 'BNCI2014001':
        ...
        # BCICIV2a preprocessed like BNCI2014001 but keeping all 4 classes.
        # X.npy already holds T-session only: 9 subjects x 288 trials = 2592,
        # ordered subject-by-subject, integer labels 0..3.
        paradigm = 'MI'
        num_subjects = 9
        sample_rate = 250
        ch_num = 22
        ...
        sample_rate = 250
        ch_num = 19
        # 每个受试者100 trials (50 left_hand + 50 right_hand), 单session
        # 数据已按 subject(1..54) -> trial 顺序排列, 无需再筛选
        y[np.where(y == 'left_hand')] = 0
        y[np.where(y == 'right_hand')] = 1
        y = y.astype(int)
        if args.backbone == 'deep':
        ...
        # 检查实际数据大小
        actual_total = X.shape[0]
        trials_per_subject = actual_total // num_subjects

        print(f"BNCI2014002: {actual_total} trials total, {trials_per_subject} per subject")

        # 支持不同的数据配置:
        # - 100 trials/subject: 仅训练数据(T)
        # - 160 trials/subject: 训练+评估数据(T+E)
        # - 其他: 可能是旧版预处理

        # 直接使用所有数据，不做索引筛选
        print(f"  使用全部数据: {actual_total} trials")
