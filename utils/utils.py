...
from sklearn.metrics import average_precision_score, cohen_kappa_score, f1_score, recall_score
...
def compute_metrics(true, all_output, args=None):
    """Compute a set of classification metrics from softmax outputs.

    Args:
        true: 1-D tensor/array of ground-truth integer labels.
        all_output: [N, class_num] tensor of softmax probabilities.
        args: namespace (uses args.class_num when available).

    Returns:
        dict with keys: acc, bca, auc, auprc, kappa, f1, recall (percentages,
        except kappa which is in [-1, 1]).
    """
    true = np.asarray(tr.as_tensor(true).cpu()).astype(int)
    probs = np.asarray(tr.as_tensor(all_output).detach().cpu())
    pred = probs.argmax(axis=1)

    class_num = getattr(args, 'class_num', probs.shape[1]) if args is not None else probs.shape[1]

    acc = accuracy_score(true, pred)
    bca = balanced_accuracy_score(true, pred)

    # F1 / Recall: binary uses positive class, multiclass uses macro average.
    avg_mode = 'binary' if class_num == 2 else 'macro'
    f1 = f1_score(true, pred, average=avg_mode, zero_division=0)
    recall = recall_score(true, pred, average=avg_mode, zero_division=0)
    kappa = cohen_kappa_score(true, pred)

    # AUC / AUPRC need probability scores. Guard against a single class present.
    try:
        if class_num == 2:
            ...
            true_onehot = np.eye(class_num)[true]
            auprc = average_precision_score(true_onehot, probs, average='macro')
    except ValueError:
        # Only one class present in y_true -> AUC/AUPRC undefined.
        auc = float('nan')
        auprc = float('nan')
...
def cal_acc_metrics_comb(loader, model, flag=True, fc=None, args=None):
    """Like cal_acc_comb, but computes the full metric set in a single pass.

    Gathers outputs and labels together so metrics stay aligned even when the
    loader is shuffled. Returns (acc, metrics_dict, all_output) where acc matches
    cal_acc_comb (bca for ERP, else accuracy), both as percentages.
    """
    start_test = True
    model.eval()
    with tr.no_grad():
        ...
        for i in range(len(loader)):
            ...
            if start_test:
                ...
                all_label = tr.cat((all_label, labels.float()), 0)
    all_output = nn.Softmax(dim=1)(all_output)
    metrics = compute_metrics(all_label, all_output, args=args)
    acc = metrics['bca'] if args.paradigm == 'ERP' else metrics['acc']
    return acc, metrics, all_output
